...
import nuevoCarrito from "../services/cart-manager";
...
carritoRouter.get('/:cid', async (req, res) => {
    const carrito = await nuevoCarrito.getCart(Number(req.params.id))
    if (carrito.length != 0)
...
carritoRouter.post('/:cid/product/:pid', async (req, res) => {
    ...
    if (carrito.length != 0) {
        const prod = await nuevoCarrito.addPorductToCart(Number(cid), Number(pid))
